...
The manifest registers one `hooks/session-start.py` entrypoint. It runs installed `hooks/session-start.d/XX-name.py` stages in filename order: attachment validation, checkout maintenance, then orientation. The stages share state and the coordinator emits one JSON object.
...
Session-start validates that the attachment is its own Git checkout and reports missing jq.
It skips refresh when local edits exist or no origin is configured. Otherwise it fetches and
attempts a fast-forward before reading orientation. Refresh failures and skips reach the model
alongside local memory; successful refresh is also stated. Network commands are bounded and
noninteractive. There is no separate remote-access, expected-URL, or push-permission probe.
...
`wiki-init`, `wiki-ask`, and `wiki-enroll` carry `disable-model-invocation: true` because each one runs a script with side effects that the user should be the one to trigger.
...
`hookSpecificOutput.additionalContext` from `hooks/session-start.py` reaches the model.
...
The shared coordinator does not read stdin, so Codex's superset SessionStart payload does not change its input contract.
...
The adapter exports `CLAUDE_PLUGIN_ROOT`, runs the shared `session-start.py` coordinator, and translates `hookSpecificOutput.additionalContext` into `additional_context`.
...
The hook process starts with its working directory at the plugin root rather than the workspace, while `session-start.py` resolves the project from its working directory, so the adapter changes directory first: `CURSOR_PROJECT_DIR`, then the payload's `workspace_roots[0]`, then the inherited working directory.
...
The key is therefore stripped from the Cursor subtree, which makes `wiki-init`, `wiki-ask`, and `wiki-enroll` model-invocable there.
