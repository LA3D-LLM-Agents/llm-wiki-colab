...
#   1. Generate .llm-wiki/Card_<repo>.md from interactive prompts
...
What it does (v0.1.0 — ask-only MVP):
  1. Generates .llm-wiki/Card_<repo>.md if absent (interactive
...
if [[ ! -d "$WIKI_DIR" ]]; then
  ...
  echo "       Run /wiki-init first to attach or create the wiki." >&2
  exit 9
fi
...
if [[ "$REPO_OWNER" == "LA3D-LLM-Agents" ]]; then
  ...
  if [[ "$OWNER_TRUSTED" -eq 1 ]]; then
    ...
    echo "up this repo (provided .llm-wiki/Card_<repo>.md exists)." >&2
  else
    echo "" >&2
