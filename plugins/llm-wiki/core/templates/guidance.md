...
**Finish the cycle: every wiki edit ends with a commit** in `.llm-wiki/`, which is its own git repo:
...
User-invocable skills: `/wiki-init` (attach or create the wiki), `/wiki-experiment`, `/wiki-source`, `/wiki-lint`. The proactive behavior above is the default; these skills exist to force an action explicitly.
