...
> `scripts/init-wiki.sh` has been adapted for the plugin model (memory at `.llm-wiki/`, no `CLAUDE.md` writer, no WIKI-INDEX registration, auto-adds the `.gitignore` line) and smoke-tested in create mode.
