...
```bash
bash "${CURSOR_PLUGIN_ROOT}/core/scripts/agent-comms/ask.sh" <agent-id> "<question>"
```
...
1. Render the candidate list for its table only (ignore the harmless `No selection. Nothing asked.` tail, you are not using the picker):
   ```bash
   bash "${CURSOR_PLUGIN_ROOT}/core/scripts/agent-comms/ask.sh" "<question>"
   ```
