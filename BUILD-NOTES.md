...
3. **Packaging drift guard**: `core/` is committed into the adapter and installs correctly, but root `core/` and `adapters/claude-code/core/` can drift. Add a pre-commit/CI check that they are in sync (run `package.sh` and fail on a dirty tree), and confirm `${CLAUDE_PLUGIN_ROOT}/core/...` resolves in a real installed session.
