...
- **Minor polish done**: `ensure-wiki.py`'s clone staging dir is renamed `.llm-wiki-clone-*` and covered by a `.llm-wiki-clone-*` gitignore line (added by both `ensure-wiki.py` and `init-wiki.sh`), so it is never briefly tracked. `core/templates/guidance.md` (the SessionStart-injected memory-boundary / wiki-maintenance text) was rewritten for the plugin context: no `CLAUDE.md`/setup.sh sentinels, `.llm-wiki/` paths, plugin gate and command references, no `${REPO_NAME}` placeholders.
